...
export function TaskBtns({ taskData, handleDelete, completeSubtask, type }) {
    return (
        <div className="task__btns-inner">
            ...
            {type === 'subtask' && < MdCheckCircleOutline className='task__btn __green' onClick={() => completeSubtask(taskData)} />}
            {type === 'task' && (<Link className='task__btn __white' to={`/task/${taskData?._id}`}> <MdNavigateNext /></Link>)}
        </div>
    );
}
